...
- [ ] Add a failing integration test that logs in as `20000000`, calls `GET /api/v1/stores/{storeId}/tenant-admin/staff/me`, and expects `accountType = tenant_admin`, `self = true`, and employee number `20000000`.
- [ ] Extend the test to patch `staff/me` with name, phone, email, and password `ADM123`, then verify login works with `adm123`.
- [ ] Add a failing integration assertion that `PATCH /staff/{tenantAdminAccountId}` returns `STAFF_NOT_FOUND`, proving normal staff endpoints do not update tenant admins.
- [ ] Add a failing integration assertion that staff user `1000` calling `GET /staff/me` returns `FORBIDDEN`.
...
- [ ] Add `CurrentActor` access from the controller or resolver so the service receives the authenticated account id.
- [ ] Add `getCurrentTenantAdmin(scope, actorId)` and `updateCurrentTenantAdmin(scope, actorId, command)` service methods.
- [ ] Add repository methods that select/update `auth_accounts` where `id = actorId`, `tenant_id = scope.tenantId`, `actor_type = 'tenant_admin'`, and active store access includes `scope.storeId`.
- [ ] Add additive response fields `accountType`, `self`, `editable`, and `statusEditable`.
- [ ] Reject self-update payloads that try to change `employeeNo` or `status`; accept name, phone, email, and optional valid password.
...
- [ ] Extend `TenantAdminStaff` with `accountType`, `self`, `editable`, and `statusEditable`.
- [ ] Add `getCurrentTenantAdminStaff(storeId)` and `updateCurrentTenantAdminStaff(storeId, request)`.
- [ ] Add route `/stores/:storeId/admin/staff/me/edit` named `tenant-admin-staff-self-edit`.
...
- [ ] Make the staff list load the self-admin row and render it above ordinary staff.
- [ ] Route protected admin row edit action to `tenant-admin-staff-self-edit`.
- [ ] In self-edit mode, load/save through `staff/me`, keep employee number read-only, and hide or disable status editing.
- [ ] Add static UI validation tests for the self-admin row, self route, and self endpoint calls.
...
- [ ] Write release notes with no-migration statement, permission impact, risk, and rollback.
- [ ] Run `mvn "-Dtest=TenantAdminApiIntegrationTest,AuthLoginUiValidationTest" test`.
- [ ] Run `npm run build`.
- [ ] Run `git diff --check`.
