...
function openPaymentReportWindow(): void {
  if (!storeId.value) {
    return
  }
  const href = router.resolve({
  ...
  }).href || `/stores/${storeId.value}/payments/report/${encodeURIComponent(normalizedTerminalCode.value)}`
  const target = `rpb-paynow-report-${storeId.value}-${normalizedTerminalCode.value}`
  reportWindow = window.open(href, target, 'popup=yes,width=520,height=900') || reportWindow
  try {
    reportWindow?.focus()
  } catch {
    // Some browsers block focusing named windows; the report page remains available.
  }
}
    ...
    >
      <template #action>
        <div class="topbar-actions">
          ...
          <button class="display-button" type="button" @click="openPaymentReportWindow">
            {{ gt('generated.payment-quick-pay.065') }}
          </button>
