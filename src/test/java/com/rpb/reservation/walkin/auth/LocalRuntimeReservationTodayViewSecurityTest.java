...
class LocalRuntimeReservationTodayViewSecurityTest {
    ...
    private static final String TIME_SLOTS_ENDPOINT = "/api/v1/stores/{storeId}/reservations/time-slots";
    ...
    @Test
    void acceptsLocalProfileTimeSlotsRequestWithoutJwtLoginWhenConfiguredActorHasCreatePermission() throws Exception {
        when(appGateService.evaluate(any())).thenReturn(AppGateDecision.allow(
            "reservation_queue",
            TENANT_ID,
            STORE_ID,
        ...
        when(applicationService.getTimeSlots(any())).thenReturn(ReservationTimeSlotListResult.success(
            STORE_ID,
            LocalDate.parse("2030-06-20"),
            "Asia/Singapore",
        ...
        mockMvc.perform(get(TIME_SLOTS_ENDPOINT, STORE_ID).param("businessDate", "2030-06-20"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.success").value(true))
            .andExpect(jsonPath("$.slots[0].periodKey").value("lunch"));

        verify(applicationService).getTimeSlots(any());
    }
}
