...
class PayNowPaymentUiAcceptanceValidationTest {
    ...
    void payNowPagesUseRpbNativePaymentApiAndQrComponent() throws Exception {
        ...
        assertThat(proofReview)
            ...
            .contains("if (result.checks?.amount === 'match')")
            .contains("const matchedAmount = formatSpokenAmount(result.ocr?.extractedAmount || null)")
            .contains("if (matchedAmount)")
            .contains("return matchedAmount")
            .contains("return formatSpokenAmount(candidate?.amount || result.expectedAmount || null)")
            .doesNotContain("candidate?.amount || result.ocr?.extractedAmount")
