...
class ReservationShareInfoApiIntegrationTest {
    ...
    void publicTokenEndpointReturnsCustomerSafeShareViewWithoutActor() throws Exception {
        ...
        mockMvc.perform(get(PUBLIC_ENDPOINT, token))
            ...
            .andExpect(jsonPath("$.share.shareText").value(org.hamcrest.Matchers.containsString("门店：食刻订位中心")))
            .andExpect(jsonPath("$.share.shareText").value(org.hamcrest.Matchers.containsString("编号：R-SHARE-0007")))
            .andExpect(jsonPath("$.share.shareText").value(org.hamcrest.Matchers.containsString("时间：20-06-2030 11:30")))
            .andExpect(jsonPath("$.share.shareText").value(org.hamcrest.Matchers.containsString("桌位：A01")))
            .andExpect(jsonPath("$.share.shareText").value(org.hamcrest.Matchers.containsString("提示：请提前 10 分钟到店")))
