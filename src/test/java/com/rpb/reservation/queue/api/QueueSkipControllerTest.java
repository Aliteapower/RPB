...
class QueueSkipControllerTest {
    ...
    private static final UUID ACTOR_ID = UUID.fromString("30000000-0000-0000-0000-000000000984");
    ...
    private static final Set<String> QUEUE_API_FILES = Set.of(
    ...
    @BeforeEach
    void setUp() {
        applicationService = mock(QueueSkipApplicationService.class);
        actorProvider = new MutableCurrentActorProvider(actor(Set.of("store_staff"), Set.of("queue.skip"), Set.of(STORE_ID)));
        mockMvc = MockMvcBuilders
            .standaloneSetup(new QueueSkipController(
                applicationService,
                actorProvider,
                new QueueSkipApiMapper(),
                new QueueSkipApiErrorMapper()
            ))
            .build();
    }

    @Test
    void skipsCalledQueueTicketAndMapsRequestToCommand() throws Exception {
        when(applicationService.skipQueueTicket(any())).thenReturn(success(false));
        ...
        mockMvc.perform(post(ENDPOINT, STORE_ID, QUEUE_TICKET_ID)
            ...
            .andExpect(jsonPath("$.reservationCode").value("R-SKIP-0984"))
        ...
        ArgumentCaptor<SkipQueueTicketCommand> commandCaptor = ArgumentCaptor.forClass(SkipQueueTicketCommand.class);
        verify(applicationService).skipQueueTicket(commandCaptor.capture());
        SkipQueueTicketCommand command = commandCaptor.getValue();
        ...
        assertThat(command.idempotencyKey()).isEqualTo("idem-skip");
        assertThat(command.actorId()).isEqualTo(ACTOR_ID);
        assertThat(command.actorType()).isEqualTo("staff");
        assertThat(command.skippedAt()).isEqualTo(SKIPPED_AT);
        assertThat(command.reasonCode()).isEqualTo("NO_RESPONSE");
        assertThat(command.note()).isEqualTo("Customer did not return");
    }
    ...
    void nullBodyUsesOnlyPathHeaderAndActorScope() throws Exception {
        ...
        ArgumentCaptor<SkipQueueTicketCommand> commandCaptor = ArgumentCaptor.forClass(SkipQueueTicketCommand.class);
        verify(applicationService).skipQueueTicket(commandCaptor.capture());
        assertThat(commandCaptor.getValue().skippedAt()).isNull();
        assertThat(commandCaptor.getValue().reasonCode()).isNull();
        assertThat(commandCaptor.getValue().note()).isNull();
    }

    @Test
    void alreadySkippedReturnsOkWithoutEvents() throws Exception {
        when(applicationService.skipQueueTicket(any())).thenReturn(QueueSkipResult.alreadySkipped(
            QUEUE_TICKET_ID,
            12,
            RESERVATION_ID,
            "R-SKIP-0984",
        ...
        mockMvc.perform(post(ENDPOINT, STORE_ID, QUEUE_TICKET_ID)
                .header("Idempotency-Key", "idem-already-skipped")
                .contentType(MediaType.APPLICATION_JSON)
                .content("{}"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.queueTicketStatus").value("skipped"))
    ...
    void completedIdempotencyReplayReturnsOkAndReplayedTrue() throws Exception {
        when(applicationService.skipQueueTicket(any())).thenReturn(success(true));
    ...
    void missingIdempotencyKeyReturnsValidationErrorWithoutCallingService() throws Exception {
        mockMvc.perform(post(ENDPOINT, STORE_ID, QUEUE_TICKET_ID)
            ...
            .andExpect(jsonPath("$.error.messageKey").value("queue.skip.missing_idempotency_key"))
    ...
    void requestDtoOnlyExposesAllowedFields() {
        assertThat(SkipQueueTicketRequest.class.getRecordComponents())
            .extracting(component -> component.getName())
            .containsExactly("skippedAt", "reasonCode", "note");
    }
    ...
    void boundaryArtifactsRemainLimitedToApprovedQueueApis() throws Exception {
        try (var paths = Files.walk(Path.of("src", "main", "java", "com", "rpb", "reservation", "queue", "api"))) {
            assertThat(paths.filter(Files::isRegularFile).map(path -> path.toString().replace('\\', '/')).toList())
                .containsExactlyInAnyOrderElementsOf(QUEUE_API_FILES);
        }
    ...
    @Test
    void endpointDeclaresAppGateReservationQueueSkipPermission() throws Exception {
        Method method = QueueSkipController.class.getMethod(
            "skipQueueTicket",
            UUID.class,
            UUID.class,
            String.class,
            SkipQueueTicketRequest.class
        );
        ...
        assertThat(annotation.permission()).isEqualTo("queue.skip");
    }

    @Test
    void mapsApplicationErrorsToApiErrors() throws Exception {
        assertApplicationError(QueueSkipError.INVALID_COMMAND, 400, "INVALID_COMMAND", "failed");
        assertApplicationError(QueueSkipError.MISSING_IDEMPOTENCY_KEY, 400, "MISSING_IDEMPOTENCY_KEY", "failed");
        assertApplicationError(QueueSkipError.STORE_NOT_FOUND, 404, "STORE_NOT_FOUND", "failed");
        assertApplicationError(QueueSkipError.STORE_SCOPE_MISMATCH, 403, "STORE_SCOPE_MISMATCH", "failed");
        assertApplicationError(QueueSkipError.STORE_ACCESS_DENIED, 403, "FORBIDDEN", "failed");
        assertApplicationError(QueueSkipError.QUEUE_TICKET_NOT_FOUND, 404, "QUEUE_TICKET_NOT_FOUND", "failed");
        assertApplicationError(QueueSkipError.QUEUE_TICKET_STATUS_NOT_CALLED, 409, "QUEUE_TICKET_STATUS_NOT_CALLED", "failed");
        assertApplicationError(QueueSkipError.QUEUE_SKIP_EVIDENCE_INCOMPLETE, 409, "QUEUE_SKIP_EVIDENCE_INCOMPLETE", "failed");
        assertApplicationError(QueueSkipError.RESERVATION_NOT_FOUND, 404, "RESERVATION_NOT_FOUND", "failed");
        assertApplicationError(QueueSkipError.RESERVATION_STATUS_NOT_ARRIVED, 409, "RESERVATION_STATUS_NOT_ARRIVED", "failed");
        assertApplicationError(QueueSkipError.IDEMPOTENCY_IN_PROGRESS, 409, "IDEMPOTENCY_IN_PROGRESS", "started");
        assertApplicationError(QueueSkipError.FAILED_IDEMPOTENCY_REQUIRES_NEW_KEY, 409, "IDEMPOTENCY_FAILED_REQUIRES_NEW_KEY", "failed");
        assertApplicationError(QueueSkipError.IDEMPOTENCY_CONFLICT, 409, "IDEMPOTENCY_CONFLICT", "conflict");
        assertApplicationError(QueueSkipError.ILLEGAL_STATE_TRANSITION, 409, "ILLEGAL_STATE_TRANSITION", "failed");
        assertApplicationError(QueueSkipError.BUSINESS_EVENT_WRITE_FAILED, 500, "EVENT_WRITE_FAILED", "failed");
        assertApplicationError(QueueSkipError.STATE_TRANSITION_WRITE_FAILED, 500, "STATE_TRANSITION_WRITE_FAILED", "failed");
        assertApplicationError(QueueSkipError.AUDIT_WRITE_FAILED, 500, "AUDIT_WRITE_FAILED", "failed");
        assertApplicationError(QueueSkipError.PERSISTENCE_ERROR, 500, "PERSISTENCE_ERROR", "failed");
    }

    @Test
    void forbiddenRoleReturnsForbiddenWithoutCallingService() throws Exception {
        actorProvider.actor = actor(Set.of("customer"), Set.of("queue.skip"), Set.of(STORE_ID));
    ...
    @Test
    void missingQueueSkipPermissionReturnsForbiddenWithoutCallingService() throws Exception {
    ...
    private void assertApplicationError(
        QueueSkipError applicationError,
        int expectedStatus,
        String expectedApiCode,
        String expectedIdempotencyStatus
    ) throws Exception {
        QueueSkipResult result = applicationError == QueueSkipError.IDEMPOTENCY_IN_PROGRESS
            ? QueueSkipResult.retryLater(applicationError)
            : QueueSkipResult.failure(applicationError);
        when(applicationService.skipQueueTicket(any())).thenReturn(result);
    ...
    private static QueueSkipResult success(boolean replayed) {
        if (replayed) {
    ...
    private static String messageKey(String apiCode) {
        return "queue.skip." + apiCode.toLowerCase();
    }
