...
public record SeatCalledQueueTicketRequest(
    ...
    public Optional<SeatingFromCalledQueueApiErrorCode> validateContract() {
        ...
        if (temporarySelectionProvided && temporaryTableIds.size() < 2) {
            return Optional.of(SeatingFromCalledQueueApiErrorCode.TEMPORARY_TABLE_GROUP_MEMBER_REQUIRED);
        }
        if (temporarySelectionProvided && new HashSet<>(temporaryTableIds).size() != temporaryTableIds.size()) {
            return Optional.of(SeatingFromCalledQueueApiErrorCode.TEMPORARY_TABLE_GROUP_MEMBER_DUPLICATE);
        }
